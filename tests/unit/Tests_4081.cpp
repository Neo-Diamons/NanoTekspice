...
static auto gate = std::make_shared<nts::Gates4081>();
...
Test(gate4081, default_state, .init = setup4081, .fini = terminate4081)
{
    cr_assert_eq(gate->compute(3), nts::Undefined);
}

Test(gate4081, input_false, .init = setup4081, .fini = terminate4081)
{
...
Test(gate4081, input_true, .init = setup4081, .fini = terminate4081)
{
...
Test(gate4081, input_undefined, .init = setup4081, .fini = terminate4081)
{
...
Test(gate4081, input_true_false, .init = setup4081, .fini = terminate4081)
{
    in_1->setState(nts::True);
    in_2->setState(nts::False);
    in_3->setState(nts::True);
...
Test(gate4081, input_false_true, .init = setup4081)
{
    in_1->setState(nts::False);
    in_2->setState(nts::True);
    in_3->setState(nts::False);
...
Test(gate4081, invalid_simulate, .init = setup4081)
{
    in_1->setState(nts::False);
    in_2->setState(nts::False);
    in_3->setState(nts::False);
...
Test(gate4081, invalid_pin, .init = setup4081)
{
    try {
        gate->compute(15);
    } catch (nts::ExceptionInvalidPin &e) {
        cr_assert_str_eq(e.what(), "4081: Invalid pin");
    }
}
