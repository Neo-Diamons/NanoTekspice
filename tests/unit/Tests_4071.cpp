...
static auto gate = std::make_shared<nts::Gates4071>();
...
Test(gate4071, default_state, .init = setup4071, .fini = terminate4071)
{
    cr_assert_eq(gate->compute(3), nts::Undefined);
}

Test(gate4071, input_false, .init = setup4071, .fini = terminate4071)
{
...
Test(gate4071, input_true, .init = setup4071, .fini = terminate4071)
{
...
Test(gate4071, input_undefined, .init = setup4071, .fini = terminate4071)
{
...
Test(gate4071, input_true_false, .init = setup4071, .fini = terminate4071)
{
    in_1->setState(nts::True);
    in_2->setState(nts::False);
    in_3->setState(nts::True);
...
Test(gate4071, input_false_true, .init = setup4071)
{
    in_1->setState(nts::False);
    in_2->setState(nts::True);
    in_3->setState(nts::False);
...
Test(gate4071, invalid_simulate, .init = setup4071)
{
    in_1->setState(nts::False);
    in_2->setState(nts::False);
    in_3->setState(nts::False);
...
Test(gate4071, invalid_pin, .init = setup4071)
{
    try {
        gate->compute(15);
    } catch (nts::ExceptionInvalidPin &e) {
        cr_assert_str_eq(e.what(), "4071: Invalid pin");
    }
}
